...
2. [Run the Generate Core Data GitHub Action](https://github.com/osrf/buildfarm-tools-private/actions/workflows/generate-core-data.yml) with `push-changes` enabled — it regenerates the Gazebo dashboard (`Gazebo.md`) as part of its normal run, there's no separate dashboard-only workflow anymore
3. Update [`buildfarm-tools/database/scripts/lib/job_priorities.csv`](https://github.com/osrf/buildfarm-tools/blob/main/database/scripts/lib/job_priorities.csv) (e.g., https://github.com/osrf/buildfarm-tools/pull/148). Steps to follow:
    ...
    3. Copy priorities from the sheet to a temp file, e.g., `/tmp/job_priorities.csv` (the first column should be the job name and the last one the job priority).
    4. Fix the format of pasted elements to be in the same format as the csv file
        * Run `awk '{print $1 "," $NF}' /tmp/job_priorities.csv > job_priorities_new_gazebo.csv`
